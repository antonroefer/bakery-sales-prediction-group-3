...
sales_kiwo = umsatzdaten.merge(kiwo, on="Datum", how="outer")

# # Create all date-warengruppe combinations
all_dates = pd.DataFrame({"Datum": sales_kiwo["Datum"].unique()})
all_warengruppen = pd.DataFrame({"Warengruppe": range(1, 7)})
all_combinations = all_dates.merge(all_warengruppen, how="cross")

# # Merge combinations with sales data
expanded_sales = all_combinations.merge(
    sales_kiwo, on=["Datum", "Warengruppe"], how="left"
)

# # Now merge with weather data based ONLY on the date
merged_df = expanded_sales.merge(wetter, on="Datum", how="left")
...
df["Schulferien"] = 0  # Standardwert: kein Feiertag
...
for start, end in ferien:
    ...
    df.loc[mask, "Schulferien"] = 1

    # Export df as Schulferien.csv without the 'id' column
    df.drop(columns=["id"], errors="ignore").to_csv("Schulferien.csv", index=False)
...
merged_df["Schulferien"] = merged_df["Schulferien"].fillna(0).astype(int)
